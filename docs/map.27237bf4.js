...
function closePanel() {
  d3.select("#info_panel").html("").transition().duration(1000).style("opacity", 0).style("height", "0 px");
...
function mainScreen(pointData) {
  ...
  return "Location:" + pointData.location + "<br>Date: " + pointData.date + "</h4>" + "<h5>" + pointData.summary + "</h5><br><div class =stat>" + "<div class=cat>Fatalties<br><h6>" + fat + "</h6></div>" + "<div class=count>" + victimCount(fat, "darkred") + "</div></div><br><div class=stat>" + "<div class=cat>Injured<br><h6>" + inj + "</h6></div>" + "<div class=count>" + victimCount(inj, "black") + "</div></div><br><br>";
}

function altScreen(data) {
  return "<h5>Weapons used: <b>" + data.weapon_type + "<br></b>Obtained legally: <b>" + data.weapons_obtained_legally + "</b><br><br><p>Suspect:<b> " + data.summary.split(",")[0] + "</b></p><ul>" + "<li>Age:" + data.age_of_shooter + "<li>Race: " + data.race + "<li>Mental health issues: " + data.prior_signs_mental_health_issues + "</ul></h5>";
}

function changeScreen(data, flag) {
  var content = flag ? altScreen(data) : mainScreen(data);
  var label = flag ? "Back" : "More Info";
...
function openPanel(pointData) {
  ...
  var current = true;
  info.append("div").attr("class", "title").attr("id", "panel_title").html("<p>" + pointData.case + "<hr width=95%>");
  info.append("div").attr("class", "info").attr("id", "panel_main").html(mainScreen(pointData));
  var nav = info.append("div").attr("class", "nav");
  nav.append("button").attr("class", "btn").attr("id", "panel_nav").text("More Info").on("click", function () {
    changeScreen(pointData, current);
    current = !current;
  });
  nav.append("button").text("Close").attr("class", "btn").on("click", closePanel);
  ...
  info.style("opacity", 0).transition().duration(500).style("opacity", 1);
}
